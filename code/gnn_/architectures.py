...
from nn_.embedding import (
    ClassEmbedding, ContinuousSinusoidalPE, define_embedding)
from generative.conditioning import FiLMHybrid, CrossAttentionBlock
from gnn_.utils import (
    extract_triu_node_data, extract_triu_edge_data, mirror_triu_to_tril_again)


class GNNDenoiserFiLMed(nn.Module):
    def __init__(
...
class GNNDenoiserCrossAttended(nn.Module):
    def __init__(
        self, node_feat: int, edge_feat: int, node_hidden: int,
        edge_hidden: int, embedding_size: int, n_layers: int=1,
        updater_layers: int=2, updater_size: int=4, ca_heads: int=1,
        input_type: str="tokens", message_passing: str="gineconv",
        act_fun: nn.Module=nn.GELU, loader=None, conditioning_args: List=[]
    ) -> None:
        super().__init__()
        ...
        self.edge_embedders = nn.ModuleList()
        ...
        embeddings = [
            define_embedding(name, **kwargs)
            for name, kwargs in conditioning_args
        ]
        ...
        for k in range(self.n_layers):
            ...
            self.node_embedders.append(
                CrossAttentionBlock(
                    ca_heads, node_hidden, embedding_size,
                    *deepcopy(embeddings), fc_projection_mult=updater_size,
                    act_fun=act_fun
                )
            )
            self.edge_embedders.append(
                CrossAttentionBlock(
                    ca_heads, edge_hidden, embedding_size,
                    *deepcopy(embeddings), fc_projection_mult=updater_size,
                    act_fun=act_fun
                )
            )
    ...
    def forward(
        ...
        node_args, edge_args = [], []
        for a in [t, y, batch.get("conditioning")] + list(args):
            if a is not None:
                if a.dim() > 1:
                    for i in range(a.size(-1)):
                        node_args.append(a[batch.batch, i])
                        edge_args.append(a[upper_graph, i])
                else:
                    node_args.append(a[batch.batch])
                    edge_args.append(a[upper_graph])
        ...
        for k in range(self.n_layers):
            ...
            # 2.2.3 Embed Conditioning onto Edges using CA
            triu_e_update = self.edge_embedders[k](triu_e_update, *edge_args)
            # 2.2.4 Residual Connection & Layer Normalization
            h_triu_e = self.edge_norms[k](h_triu_e + triu_e_update)
            # 2.2.5 Mirror Final Features Back to "Full Adjacency"
            hid_edge = mirror_triu_to_tril_again(
